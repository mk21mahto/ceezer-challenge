...
const Cart = () => {
  ...
  const totalPrice = cartItem.reduce((acc, currentItem) => {
    const productTotal = currentItem.basePrice * currentItem.quantity;
    return acc + productTotal;
  }, 0);

    // Function to remove an item from the cart based on its name
    const removeItem = (nameToRemove: string) => {
      const updatedCart = cartItem.filter(item => item.name !== nameToRemove);
      setCartItem(updatedCart);
    };

    const updateQuantity = (nameToUpdate, newQuantity) => {
                  ...
                  {cartItem.map((item) => (
                      <li key={item.name} className="flex py-6">
                        <div className="h-24 w-24 flex-shrink-0 overflow-hidden rounded-md border border-gray-200">
                          <Image src={item.image} alt='image' width={100} height={50}
                        ...
                        <div className="ml-4 flex flex-1 flex-col">
                          ...
                          <div className="flex flex-1 items-end justify-between text-sm">
                            ...
                            <input value={item.quantity} className='w-6 border' onChange={(e) => {updateQuantity(item.name, e.target.value)}}></input>
                            </p>

                            <div className="flex">
                              <button type="button" onClick={() => {removeItem(item.name)}} className="font-medium text-indigo-600 hover:text-indigo-500">
                                Remove
                              </button>
                            </div>
                          </div>
